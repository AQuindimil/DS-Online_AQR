import random
import numpy as np
from Variables_3 import (
    FILAS_TABLERO, COLUMNAS_TABLERO,
    BARCOS,
    AGUA, BARCO, IMPACTO, FALLO,
    SIMBOLOS
)
import os #importo esto pmara poder limpiar la pantalla en consola depsués del while true
...
class Juego:
    ...
    def iniciar(self):
        ...
        while True:
            ...
            if turno == "jugador":
                ...
                print("Impacto! 💥" if acierto else "Agua! 💧") #para saber que pasa en el turno, auqnue se vea
                if self.maquina.vidas == 0:
                    print("\n¡Has ganado! Todos los barcos enemigos han sido hundidos.")
                    break
                turno = "jugador" if acierto else "maquina"

                time.sleep(3) #el sleep hace que no vaya tan del tirón el juego y le aporta naturalidad :)
                ...
                print("Impacto! 💥" if acierto else "Agua! 💧") #LO MISMO qu arriba
    ...
    def pedir_coordenadas_usuario(self):
        from  funciones_3 import pedir_coordenadas
            
        disparos_previos={
            (x,y)
            for x in range(self.filas)
            for y in range(self.columnas)
            if self.jugador.tablero.disparos[x,y] != AGUA
            }
        return pedir_coordenadas(self.filas, self.columnas, disparos_previos)
